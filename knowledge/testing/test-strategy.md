...
- **Embedded presentation:** `node scripts/render_check.js --asset-selftest`
  ...
  The 24 stable JSON files in `tests/fixtures/ui/` are generated by the
  ...
  `--all-states` runs 58 named rows against the served application: exact ordered requests,
...
its served page/assets, and fulfills the 24 Rust-generated typed JSON
fixtures in `tests/fixtures/ui/` at the API boundary. `--all-states` walks the
58 named request/DOM/clean-run interactions (including loading via a held
